...
|---------|---------|---------------------|
| API Gateway | Partial — NGINX reverse proxy, not a full gateway product | Single backend service in v1; NGINX covers TLS/static/rate-limiting needs — see [`ADR-003`](decisions/records/ADR-003-nginx-reverse-proxy.md) |
| Database per Service | No | Single deployable service (`library-api`), single shared PostgreSQL DB in v1 — see [`ADR-002`](decisions/records/ADR-002-hexagonal-modular-monolith.md) |
| Circuit Breaker | No | No external service dependencies in v1 — `library-api` only calls its own database |
| Saga (choreographed) | No | All operations fit in a single service/DB transaction (ACID); no distributed transaction exists yet |
| Outbox Pattern | No | Domain events (`02-domain/domain-events.md`) are raised and handled in-process, in the same transaction — nothing is published externally |
| CQRS | No | Read and write models are the same size/shape at this scale; would only add complexity |
| Event Sourcing | No | No requirement for full state reconstruction from history; `Loan`/`Book`/`Student` store current state directly |
| BFF | No | Single client (the React SPA) with a single set of needs — a dedicated BFF would duplicate `library-api` |
